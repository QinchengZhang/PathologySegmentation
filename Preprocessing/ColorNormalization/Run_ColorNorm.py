...
def run_batch_colornorm(filenames, nstains, lamb, output_direc, img_level, background_correction=True, config=None):
    ...
    for filename in filenames:
        ...
        s = output_direc + \
            base_s.replace(".", "_")+" (using " + \
            base_t.replace(".", "_")+" "+correc+").png"
        ...
        Wi, i0 = Wfast(I, nstains, lamb, num_patches,
                       patchsize, level, background_correction)
        if i0 is None:
        ...
        if (xdim*ydim) <= (_max*_max):
            ...
            img = np.asarray(I.read_region(
                (0, 0), level, (xdim, ydim)), dtype=np.float32)[:, :, :3]

            Hiv = session1.run(
                Hiv1, feed_dict={Img1: img, Wis1: Wi, src_i_0: i0})
            ...
            session1.run(fwrite, feed_dict={shape: np.array(
                img.shape), Wit1: Wi_target, Hiv2: Hiv, sav_name: s, tar_i_0: tar_i0, normfac: norm_fac})
            ...
            if file_no == 0:
                Hivt = np.memmap('H_target', dtype='float32',
                                 mode='w+', shape=(xdim*ydim, 2))
            else:
                Hivs = np.memmap('H_source', dtype='float32',
                                 mode='w+', shape=(xdim*ydim, 2))
                sourcenorm = np.memmap(
                    'wsi', dtype='uint8', mode='w+', shape=(ydim, xdim, 3))
            ...
            for x in x_tl:
                for y in y_tl:
                    ...
                    print "\t \t Time since processing started:", round(time.time()-tic, 3)
                    img = np.asarray(I.read_region(
                        (int(ds*x), int(ds*y)), level, (xx, yy)), dtype=np.float32)[:, :, :3]
            ...
            for x in x_tl:
                for y in y_tl:
                    ...
                    sourcenorm[y:y+yy, x:x+xx, :3] = session1.run(source_norm, feed_dict={Hiv2: np.array(
                        Hivs[ind:ind+pix, :]), Wit1: Wi_target, normfac: _normfac, shape: sh, tar_i_0: tar_i0})
...
def run_colornorm(source_filename, target_filename, nstains, lamb, output_direc, level, background_correction=False, config=None):
    ...
    run_batch_colornorm(filenames, nstains, lamb, output_direc,
                        level, background_correction, config)
