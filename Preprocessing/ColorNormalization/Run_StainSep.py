...
'''
Author: TJUZQC
Date: 2020-10-11 02:12:24
LastEditors: TJUZQC
LastEditTime: 2020-11-20 19:25:53
Description: None
'''
import os
import sys
...
def run_stainsep(filename, nstains, lamb, output_direc="", background_correction=True):
    ...
    Wi, Hi, Hiv, stains = Faststainsep(
        I, img, nstains, lamb, level, background_correction)
    ...
    cv2.imwrite(output_direc+fname+"-0_original.png",
                cv2.cvtColor(img, cv2.COLOR_RGB2BGR))
    cv2.imwrite(output_direc+fname+"-1_Hstain.png",
                cv2.cvtColor(stains[0], cv2.COLOR_RGB2BGR))
    cv2.imwrite(output_direc+fname+"-2_Estain.png",
                cv2.cvtColor(stains[1], cv2.COLOR_RGB2BGR))
...
def Faststainsep(I_obj, I, nstains, lamb, level, background_correction):
    ...
    Hiv = np.transpose(np.dot(np.linalg.pinv(
        Wi), np.transpose(V)))  # Pseudo-inverse
    Hiv[Hiv < 0] = 0
    ...
    for i in range(nstains):
        vdAS = np.reshape(Hiv[:, i], (rows*cols, 1)) * \
            np.reshape(Wi[:, i], (1, 3))
        sepstains.append(
            np.uint8(i0*np.reshape(np.exp(-vdAS), (rows, cols, 3))))
